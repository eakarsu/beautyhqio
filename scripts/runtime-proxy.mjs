...
const server = http.createServer((request, response) => {
  const upstream = http.request({ hostname: '127.0.0.1', port: targetPort, path: request.url, method: request.method, headers: { ...request.headers, host: `127.0.0.1:${listenPort}`, 'x-forwarded-host': `127.0.0.1:${listenPort}`, 'x-forwarded-proto': 'http' } }, upstreamResponse => {
