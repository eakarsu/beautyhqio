...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const appointment = await prisma.appointment.findUnique({ where: { id }, include: { client: true, staff: { include: { user: { select: publicUserSelect } } }, location: true, services: { include: { service: true, addOns: { include: { addOn: true } } } }, photos: true, transaction: true } });
