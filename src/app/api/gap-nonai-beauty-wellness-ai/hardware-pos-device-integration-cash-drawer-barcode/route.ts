...
// Gap-nonai endpoint: Hardware POS device integration (cash drawer, barcode)
  ...
  try {
    ...
    const r = await fetch(OPENROUTER_URL, {
      ...
      body: JSON.stringify({ model: OPENROUTER_MODEL, messages: [
        ...
        { role: 'user', content: `Feature: Hardware POS device integration (cash drawer, barcode)\nContext: ${JSON.stringify(body)}\nReturn JSON {"summary":"","key_points":[""],"recommendations":[""]}` }
    ...
    await persist('hardware-pos-device-integration-cash-drawer-barcode', body, out);
    return NextResponse.json({ feature: 'hardware-pos-device-integration-cash-drawer-barcode', title: 'Hardware POS device integration (cash drawer, barcode)', result: out });
  } catch (err: any) {
